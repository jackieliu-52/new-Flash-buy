...
import com.jackie.flash_buy.views.plan.DetailActivity;
...
public class ZoomHeaderView extends LinearLayout {
    ...
    private void doPagerUp(float moveY, float currentY) {
        ...
        mCloseTxt.setAlpha(0f);  //隐藏文字
    }

    //根据当前viewpager y轴恢复到正常位置
    public void restore(float y) {
        ...
        //禁止滑动
        ((DetailActivity.CtrlLinearLayoutManager) mRecyclerView.getLayoutManager()).setScrollEnabled(false);
    ...
    private void expand(float y) {
        ...
        va.start();
        //允许滑动
        ((DetailActivity.CtrlLinearLayoutManager) mRecyclerView.getLayoutManager()).setScrollEnabled(true);
